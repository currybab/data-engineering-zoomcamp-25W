...
project: myproject
dataset: my_nyc_tripdata
table: ext_green_taxi

so `myproject.my_nyc_tripdata.ext_green_taxi`
...
other one must materialize `fct_taxi_monthly_zone_revenue` table
...
DBT_BIGQUERY_STAGING_DATASET is not mandatory, defaults to `DBT_BIGQUERY_TARGET_DATASET`
...
WITH valid_trips AS (
    SELECT
        ...
        pickup_month,
        fare_amount
    FROM {{ ref('fact_trips_add_dimension') }}
    WHERE
        fare_amount > 0
        AND trip_distance > 0
...
SELECT
    ...
    pickup_year,
    pickup_month,
    ROUND(p97_fare, 2) as p97_fare,
    ROUND(p95_fare, 2) as p95_fare,
    ROUND(p90_fare, 2) as p90_fare
FROM (
...
zone_stats AS (
    SELECT
        pickup_zone,
        dropoff_zone,
        PERCENTILE_CONT(trip_duration_seconds, 0.9) OVER (
